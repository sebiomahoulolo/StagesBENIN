            ...
            <div class="card-header py-3 bg-primary">
                <div class="row align-items-center">
                    ...
                    <div class="col">
                        ...
                        <p class="text-white-80 mb-0 mt-1">{{ $annonce->titre ?? ($annonce->nom_du_poste ?? 'Sans titre') }}
                        </p>
                    </div>
                    <div class="col-auto">
                        <span
                            class="badge bg-{{ $annonce->statut === 'en_attente' ? 'warning' : ($annonce->statut === 'approuve' ? 'success' : 'danger') }} px-3 py-2 fs-6">
                        ...
                        <div class="row g-4">
                            <div class="col-md-6">
                                <div class="p-3 h-100 bg-light rounded">
                                    <h5 class="font-weight-bold text-primary border-bottom pb-2 mb-3">Informations
                                        principales</h5>
                                    <ul class="list-group list-group-flush">
                                        ...
                                        <li class="list-group-item bg-transparent border-0 py-2 ps-0">
                                            ...
                                            {{ $annonce->type_contrat ?? ($annonce->type_de_poste ?? 'Non spécifié') }}
                                        </li>
                                        <li class="list-group-item bg-transparent border-0 py-2 ps-0">
                                ...
                                <div class="p-3 h-100 bg-light rounded">
                                    ...
                                    <ul class="list-group list-group-flush">
                                        <li class="list-group-item bg-transparent border-0 py-2 ps-0">
                                            ...
                                            <span
                                                class="badge bg-{{ $annonce->statut === 'en_attente' ? 'warning' : ($annonce->statut === 'approuve' ? 'success' : 'danger') }}">
                                                {{ $annonce->statut === 'en_attente' ? 'En attente' : ($annonce->statut === 'approuve' ? 'Approuvé' : 'Rejeté') }}
                                            </span>
                                        </li>
                                        <li class="list-group-item bg-transparent border-0 py-2 ps-0">
                                        ...
                                        <li class="list-group-item bg-transparent border-0 py-2 ps-0">
                                            ...
                                            {{ $annonce->date_cloture?->format('d/m/Y') ?? 'Non spécifiée' }}
                                        </li>
                                                ...
                                                <td>
                                                    <div class="btn-group">
                                                        <button type="button" class="btn btn-sm btn-outline-primary"
                                                            ...
                                                            data-bs-target="#candidatureModal{{ $candidature->id }}">
                                                            <i class="fas fa-eye"></i>
                                                        </button>
                                                        <button type="button" class="btn btn-sm btn-outline-success"
                                                            data-bs-toggle="modal"
                                                            data-bs-target="#changerStatutModal{{ $candidature->id }}">
                                                            <i class="fas fa-edit"></i>
                                                        </button>
                                                    </div>

                                                    <!-- Modal pour afficher les détails de la candidature -->
                                                    <div class="modal fade" id="candidatureModal{{ $candidature->id }}"
                                                        ...
                                                        <div class="modal-dialog modal-lg">
                                                            <div class="modal-content">
                                                                ...
                                                                <div class="modal-body">
                                                                    ...
                                                                    @php
                                                                        $cvPath = public_path(
                                                                            'assets/' . $candidature->cv_path,
                                                                        );
                                                                    @endphp

                                                                    @if ($candidature->cv_path && file_exists($cvPath))
                                                                        <div class="mb-3">
                                                                            <a href="{{ asset('assets/' . $candidature->cv_path) }}"
                                                                                target="_blank"
                                                                    ...
                                                                    @else
                                                                        <div class="mb-3 text-danger">
                                                                            Le CV n'est pas disponible ou le fichier est
                                                                            manquant.
                                                                        </div>
                                                                    @endif
                                                    ...
                                                    <div class="modal fade" id="changerStatutModal{{ $candidature->id }}"
                                                        tabindex="-1" aria-hidden="true">
                                                        <div class="modal-dialog">
                                                            <div class="modal-content">
                                                                <form
                                                                    action="{{ route('admin.candidatures.updateStatut', $candidature) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    @method('PUT')
                                                                    <div class="modal-header">
                                                                        <h5 class="modal-title">Modifier le statut de la
                                                                            candidature</h5>
                                                                        <button type="button" class="btn-close"
                                                                    ...
                                                                    <div class="modal-body">
                                                                        <div class="mb-3">
                                                                            <label for="statut{{ $candidature->id }}"
                                                                                class="form-label">Statut</label>
                                                                            <select class="form-select"
                                                                                id="statut{{ $candidature->id }}"
                                                                                name="statut" required>
                                                                                <option value="en_attente"
                                                                                    {{ $candidature->statut == 'en_attente' ? 'selected' : '' }}>
                                                                                    En attente</option>
                                                                                <option value="en_cours"
                                                                                    {{ $candidature->statut == 'en_cours' ? 'selected' : '' }}>
                                                                        ...
                                                                        <div class="mb-3"
                                                                            id="motifRejetDiv{{ $candidature->id }}"
                                                                            style="{{ $candidature->statut == 'rejete' ? '' : 'display: none;' }}">
                                                                            <label for="motif_rejet{{ $candidature->id }}"
                                                                                class="form-label">Motif de rejet</label>
                ...
                <div class="card shadow mb-4 border-left-success">
                    ...
                    <div class="card-body">
                        @if ($annonce->statut === 'en_attente')
                            <form action="{{ route('admin.annonces.approuver', $annonce) }}" method="POST"
                                class="mb-3">
                                @csrf
                        ...
                        @else
                            <div
                                class="alert alert-{{ $annonce->statut === 'approuve' ? 'success' : 'danger' }} border-0 shadow-sm">
                                <i
                                    class="fas {{ $annonce->statut === 'approuve' ? 'fa-check-circle' : 'fa-times-circle' }} fa-lg me-2"></i>
                                L'annonce a été {{ $annonce->statut === 'approuve' ? 'approuvée' : 'rejetée' }} le
                                {{ $annonce->updated_at->format('d/m/Y H:i') }}
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="modal-content border-0 shadow">
                ...
                <form action="{{ route('admin.annonces.rejeter', $annonce) }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        @endif
                        <div class="mb-3">
                            <label for="motif_rejet" class="form-label">Motif du rejet <span
                                    class="text-danger">*</span></label>
                            <textarea class="form-control @error('motif_rejet') is-invalid @enderror" name="motif_rejet" id="motif_rejet_show"
                                rows="3" required minlength="10">{{ old('motif_rejet') }}</textarea>
                            @error('motif_rejet')
                ...
                @foreach ($candidatures as $candidature)
                    document.getElementById('statut{{ $candidature->id }}')?.addEventListener('change', function() {
                        if (this.value === 'rejete') {
                            document.getElementById('motifRejetDiv{{ $candidature->id }}').style.display =
                                'block';
                        } else {
                            document.getElementById('motifRejetDiv{{ $candidature->id }}').style.display =
                                'none';
