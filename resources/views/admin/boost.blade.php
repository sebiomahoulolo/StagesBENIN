...
<div class="container-fluid mt-4"> {{-- Ajout d'un container pour une meilleure mise en page --}}
    <div class="card"> {{-- Encapsulation dans une card Bootstrap --}}
        <div class="card-header d-flex justify-content-between align-items-center">
            <h4 class="mb-0">Liste des Boosts Enregistrés (<span id="totalTiers">{{ $tiers->total() }}</span> affichés / {{ $tiers->total() }} total)</h4>
            <div class="action-buttons">
                {{-- Barre de recherche --}}
                <input type="text" id="searchInput" class="form-control d-inline-block w-auto ms-3" placeholder="Rechercher...">
            </div>
        </div>
...
<form action="{{ route('admin.boost') }}" method="POST" style="display:inline;" onsubmit="return confirm('Êtes-vous sûr de vouloir marquer ce boost comme Payé ?');">
    @csrf
    ...
    <button type="submit" class="btn btn-success btn-sm" data-bs-toggle="tooltip" title="Valider le Paiement">
        <i class="fas fa-check-circle"></i> Valider
    </button>
</form>
                                            @else
                                                 {{-- Afficher un indicateur si déjà payé --}}
                                                 <span class="text-success" data-bs-toggle="tooltip" title="Paiement déjà validé"><i class="fas fa-check-circle"></i> Payé</span>
                                            @endif
