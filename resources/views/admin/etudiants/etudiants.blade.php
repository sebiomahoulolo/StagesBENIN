    ...
    <div class="tab-content active" id="etudiants-content" role="tabpanel" aria-labelledby="etudiants-tab">
        ...
        <div class="action-bar d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="d-flex gap-2">
                ...
                <button type="button" id="btnMessage" class="btn btn-secondary" data-bs-toggle="modal"
                ...
                <div class="filter-item">
                    ...
                    <div class="input-group">
                        <input type="text" id="etudiantSearch" class="form-control"
                            placeholder="Nom, prénom, téléphone, formation...">
        ...
        <div class="stats-container">
            ...
            <div class="stat-card">
                ...
                <h5>Ce mois</h5>
                <p>{{ \App\Models\Etudiant::whereBetween('created_at', [now()->startOfMonth(), now()->endOfMonth()])->count() }}
                </p>
            </div>
        </div>
                            ...
                            <tr data-student-id="{{ $etudiant->id }}">
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        ...
                                        <button type="button"
                                            class="btn btn-{{ $etudiant->statut == 1 ? 'warning' : 'success' }} btn-sm toggle-status"
            ...
            function extractTableData() {
                ...
                originalTableData = Array.from(rows).map(row => {
                    ...
                    return {
                        ...
                        telephone: cells[2]?.textContent.trim() === '-' ? null : cells[2]?.textContent
                        .trim(),
                        niveau: cells[3]?.textContent.trim() === '-' ? null : cells[3]?.textContent.trim(),
                        formation: cells[4]?.textContent.trim() === '-' ? null : cells[4]?.textContent
                        .trim(),
            ...
            async function toggleStudentStatus(studentId, currentStatus) {
                try {
                    ...
                    if (response.ok) {
                        ...
                        updateDisplay();
                        showToast(`Statut de l'étudiant ${currentStatus == 1 ? 'bloqué' : 'activé'}`,
                        'success');
                    } else {
