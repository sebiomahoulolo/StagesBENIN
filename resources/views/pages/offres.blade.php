...
<br><br><br>
                 ...
                 <aside class="filter-section shadow-sm mb-4 mb-lg-0">
                     ...
                     <form action="{{ route('etudiants.offres.index') }}" method="GET" id="filterForm">
                         ...
                         <div class="filter-group">
                             ...
                             <select class="form-select form-select-sm select2-filter" id="secteur_id" name="secteur_id"
                                 data-placeholder="Choisir un secteur">
                         ...
                         <div class="filter-group">
                             ...
                             <select class="form-select form-select-sm" id="type_de_poste" name="type_de_poste">
                                 ...
                                 <option value="Stage" {{ request('type_de_poste') == 'Stage' ? 'selected' : '' }}>Stage
                                 </option>
                                 <option value="Emploi" {{ request('type_de_poste') == 'Emploi' ? 'selected' : '' }}>Emploi
                                 </option>
                                 <option value="Alternance"
                                     {{ request('type_de_poste') == 'Alternance' ? 'selected' : '' }}>Alternance</option>
                             </select>
                         </div>
                         ...
                         <div class="filter-group">
                             ...
                             <select class="form-select form-select-sm" id="niveau_detude" name="niveau_detude">
                                 <option value="">Tous</option>
                                 <option value="BEPC" {{ request('niveau_detude') == 'BEPC' ? 'selected' : '' }}>BEPC
                                 </option>
                                 <option value="CAP" {{ request('niveau_detude') == 'CAP' ? 'selected' : '' }}>CAP
                                 </option>
                                 <option value="BAC" {{ request('niveau_detude') == 'BAC' ? 'selected' : '' }}>BAC
                                 </option>
                                 <option value="BAC+1" {{ request('niveau_detude') == 'BAC+1' ? 'selected' : '' }}>BAC+1
                                 </option>
                                 <option value="BAC+2" {{ request('niveau_detude') == 'BAC+2' ? 'selected' : '' }}>BAC+2
                                 </option>
                                 <option value="BAC+3" {{ request('niveau_detude') == 'BAC+3' ? 'selected' : '' }}>BAC+3
                                 </option>
                                 <option value="BAC+4" {{ request('niveau_detude') == 'BAC+4' ? 'selected' : '' }}>BAC+4
                                 </option>
                                 <option value="BAC+5" {{ request('niveau_detude') == 'BAC+5' ? 'selected' : '' }}>BAC+5
                                 </option>
                                 <option value="Doctorat" {{ request('niveau_detude') == 'Doctorat' ? 'selected' : '' }}>
                                     Doctorat</option>
                             </select>
                         </div>
                 ...
                 <div class="list-header mb-4 p-3 bg-white rounded shadow-sm">
                     <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                         <h5 class="mb-2 mb-sm-0 text-primary fw-bold">
                             @if ($annonces->total() > 0)
                                 {{ $annonces->total() }} Offre{{ $annonces->total() > 1 ? 's' : '' }}
                                 trouvée{{ $annonces->total() > 1 ? 's' : '' }}
                             @else
                                 Offres disponibles
                             @endif
                         </h5>
                 ...
                 @else
                     <div class="annonces-list">
                         @foreach ($annonces as $annonce)
                             <article class="offre-card card mb-3 animate__animated animate__fadeInUp"
                                ...
                                @if ($annonce->entreprise && !$annonce->entreprise->logo) {{-- Nom de l'entreprise si pas de logo --}}
                                         ...
                                         <div class="badge-container mb-3">
                                             <span class="badge bg-primary-soft text-primary"><i
                                                     class="fas fa-briefcase me-1"></i>{{ $annonce->type_de_poste }}</span>
                                             @if ($annonce->secteur)
                                                 <span class="badge bg-success-soft text-success"><i
                                                         class="fas fa-tags me-1"></i>{{ $annonce->secteur->nom }}</span>
                                             @endif
                                             @if ($annonce->specialite)
                                                 <span class="badge bg-info-soft text-info"><i
                                                         class="fas fa-cogs me-1"></i>{{ $annonce->specialite->nom }}</span>
                                             @endif
                                             <span class="badge bg-warning-soft text-warning"><i
                                                     class="fas fa-graduation-cap me-1"></i>{{ $annonce->niveau_detude }}</span>
                                             <span class="badge bg-secondary-soft text-secondary"><i
                                                     class="fas fa-map-marker-alt me-1"></i>{{ $annonce->lieu }}</span>
                                         </div>
                                     ...
                                     <div class="card-footer bg-light-subtle border-top-0"> {{-- bg-light-subtle pour un fond très clair --}}
                                         <div class="d-flex justify-content-between align-items-center">
                                             <span class="date-cloture text-muted small">
                                                 ...
                                                 Clôture le
                                                 {{ $annonce->date_cloture ? $annonce->date_cloture->format('d/m/Y') : 'Non spécifiée' }}
                                             </span>
                                             <a href="{{ route('etudiants.offres.show', $annonce) }}"
                                                 class="btn btn-primary btn-sm btn-voir-offre">
                     ...
                     @if ($annonces->hasPages())
                         <div class="d-flex justify-content-center mt-4 pt-2">
                             <nav>
                                 {{ $annonces->appends(request()->query())->onEachSide(1)->links('pagination::bootstrap-5') }}
                             </nav>
                         </div>
                     @endif
     ...
     <script>
         document.addEventListener('DOMContentLoaded', function() {
             ...
             function loadSpecialites(secteurId, selectedSpecialiteId = null) {
                 ...
                 specialiteSelect.empty().append($('<option></option>').val("").text(placeholderText)).prop(
                     'disabled', true);
                 ...
                 if (secteurId) {
                     ...
                     fetch(
                             `/api/specialites/${secteurId}`
                             ) // Assurez-vous que cette route API existe et retourne du JSON
                         .then(response => {
                             if (!response.ok) {
                         ...
                         .then(specialites => {
                             ...
                             specialiteSelect.data('placeholder',
                                 placeholderText); // Restore original placeholder
                             ...
                             if (specialites.length > 0) {
                                 ...
                                 specialiteSelect.append($('<option></option>').val("").text(
 ...
 <style>
     :root {
         ...
         /* Bleu que vous utilisiez */
         --bs-primary: #3498db;
         --bs-primary-hover: #2980b9;
         --bs-secondary-rgb: 108, 117, 125;
         --bs-secondary: #6c757d;
         --bs-success-rgb: 34, 170, 96;
         /* Vert un peu plus doux */
         --bs-success: #22aa60;
         --bs-info-rgb: 91, 89, 182;
         /* Violet que vous utilisiez pour spécialité */
         --bs-info: #5b59b6;
         /* Corrigé de #9b59b6 */
         --bs-warning-rgb: 241, 196, 15;
         /* Jaune que vous utilisiez */
         --bs-warning: #f1c40f;
         --bs-danger-rgb: 231, 76, 60;
         /* Rouge que vous utilisiez */
         --bs-danger: #e74c3c;
     ...
     body {
         ...
         font-size: 0.95rem;
         /* Taille de police de base légèrement ajustée */
     }
     ...
     .filter-group .form-select {
         ...
         /* Bordure d'input légèrement plus visible */
         font-size: 0.875rem;
         padding: 0.4rem 0.75rem;
         /* Padding ajusté pour form-control-sm feeling */
     ...
     .filter-group .form-select:focus {
         ...
         /* Ombre focus plus subtile */
     }

     .filter-group .input-group-text {
         background-color: #f3f4f6;
         /* Fond plus clair pour l'icône */
     ...
     .filter-group .input-group .form-control:focus {
         /* Pour que l'icône change de couleur aussi au focus */
         border-left: 0;
         /* On le remet car BS peut le modifier */
     }
     ...
     .select2-container--bootstrap-5 .select2-selection {
         font-size: 0.875rem;
         min-height: calc(1.5em + (0.4rem * 2) + 2px);
         /* Match form-control-sm style */
         padding: 0.4rem 0.75rem;
         border: 1px solid #d1d5db;
         border-radius: var(--input-border-radius);
     }
     ...
     .offre-card:hover {
         ...
         /* Effet de soulèvement plus marqué */
         border-color: rgba(var(--bs-primary-rgb), 0.4);
     }

     .offre-card-title a {
         font-size: 1.1rem;
         /* Légèrement réduit pour équilibre */
         font-weight: 600;
         color: #1f2937;
         /* Titre plus foncé */
     ...
     .badge-container .badge {
         ...
         align-items: center;
         line-height: 1;
         /* Pour un meilleur alignement vertical de l'icône */
     }

     .badge-container .badge i {
         font-size: 0.85em;
         /* Taille icône relative au texte du badge */
         margin-right: 0.35em;
     }
     ...
     .fa-folder-open {
         /* Déjà bien, juste pour référence */
         font-size: 4rem;
     }
     ...
     @media (max-width: 991.98px) {
         ...
         .offre-card-title a {
             font-size: 1.05rem;
             /* Un peu plus grand sur tablette */
         }
     }
     ...
     @media (max-width: 767.98px) {
         ...
         .list-header .d-flex {
             ...
             /* Assurer que les éléments prennent toute la largeur */
             gap: 0.75rem;
             /* Espace entre titre et bouton "Mes candidatures" */
         }
         ...
         .offre-card .card-footer {
             padding: 1rem;
             /* Moins de padding dans les cartes sur mobile */
         }
