<div class="form-section">
    ...
    @forelse ($competences as $index => $competence)
        <div class="form-repeater-item mb-3" wire:key="competence-item-{{ $competence['id'] ?? $index }}">
             ...
             @else
                 ...
                 <div class="row align-items-center">
                    <div class="col">
                        <span style="font-size: 0.8em; color: var(--dark-gray); display:block;">{{ $competence['categorie'] }}</span>
                        <span style="font-weight: 500;">{{ $competence['nom'] }}</span>
                    </div>
                    <div class="col" style="flex: 0 0 150px;">
                         <div class="progress" style="height: 8px;">
                             <div class="progress-bar" role="progressbar" style="width: {{ $competence['niveau'] }}%; background-color: var(--primary-color);" aria-valuenow="{{ $competence['niveau'] }}" aria-valuemin="0" aria-valuemax="100"></div>
                         </div>
                         <span style="font-size: 0.8em; color: var(--dark-gray); text-align: right; display:block;">{{ $competence['niveau'] }}%</span>
                    </div>
                 </div>
             @endif
        </div>
    @empty
        ...
        @unless($showAddForm || $editingIndex !== null)
            <div style="text-align: center; color: var(--dark-gray); padding: 1rem; border: 1px dashed !important; grid-column: 1 / -1; border-radius: 4px;"> Aucune compétence ajoutée. </div>
        @endunless
    @endforelse
</div>