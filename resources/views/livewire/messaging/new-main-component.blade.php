    ...
    <div class="container-fluid d-flex flex-column flex-md-grow-1">
        ...
        <div class="row flex-md-grow-1">
            <div class="col-12 col-md-4 col-xxl-3 d-md-flex mw-lg-350px">
                <!-- Conversation List -->
                @livewire('messaging.new-conversation-list')
            </div>
            <div class="col d-flex min-h-500px">
                <!-- Zone principale -->
                @if($showCreateConversation && $isAdminOrEnterprise())
                    @livewire('messaging.new-create-conversation')
                @elseif($selectedConversationId)
                    <div class="card border-0 flex-grow-1">
                        @livewire('messaging.new-conversation-messages', ['conversationId' => $selectedConversationId])
                        @livewire('messaging.new-message-form', ['conversationId' => $selectedConversationId])
                    </div>
                @else
                    <div class="card border-0 flex-grow-1">
                        <div class="card-body text-center py-5">
                            <div class="py-4">
                                ...
                                <p class="text-muted">Sélectionnez une conversation dans la liste</p>
                                @if($isAdminOrEnterprise())
