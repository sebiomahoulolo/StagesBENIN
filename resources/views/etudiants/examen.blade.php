...
@section('content')
    <div class="container">
        ...
        <div id="examInterface" class="exam-interface {{ !isset($examen) || $examen->score === null ? 'active' : '' }}">
            @if (count($questions) > 0)
                <div class="exam-header">
                    <h1>🎓 QCM pour le test</h1>
                    <div class="warning-alert">
                        <strong>⚠️ Attention !</strong> Une note en dessous de 12/20 entraîne une disqualification
                        automatique.
                    </div>
                </div>
                ...
                <div class="info-section">
                    ...
                    <div class="info-item timer">
                        ...
                        <span><strong>Temps restant :</strong> <span
                                id="countdown">{{ isset($entretien) && $entretien->duree ? $entretien->duree : '30:00' }}</span></span>
                    </div>
                </div>

                <form id="examForm"
                    action="{{ route('etudiants.examen.submit', ['etudiant_id' => $etudiant->id, 'entretien_id' => $entretien->id]) }}"
                    method="POST">
                    @csrf
                    ...
                    @foreach ($questions as $question)
                        <div class="question-container mb-4">
                                    ...
                                    @foreach ($reponses as $reponse)
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio"
                                                name="reponses[{{ $question->id }}]" value="{{ $reponse->texte }}" required>
                                    ...
                                    @foreach ($reponses as $reponse)
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox"
                                                name="reponses[{{ $question->id }}][]" value="{{ $reponse->texte }}">
                                            <label class="form-check-label">{{ $reponse->texte }}</label>
                                        </div>
                                    @endforeach
                                @else
                            ...
                            @elseif ($type === 'cas_pratique')
                                <textarea name="reponses[{{ $question->id }}]" class="form-control pratique-textarea" maxlength="340" rows="4"
                                    required></textarea>
                                <small class="text-muted"><span class="caracteres-restants">340</span> caractères
                                    restants</small>
                    ...
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        textareas.forEach(function(textarea) {
                            ...
                            const updateCounter = () => {
                                ...
                                '#6c757d'; // rouge si proche de la limite
                            };
            ...
            @else
                <div class="no-exam-message active">
                    <h2>📋 Aucun test disponible</h2>
                    <p>Il n'y a actuellement aucun test ou question disponible.</p>
        ...
        <div id="loadingSpinner" class="loading-spinner">
            ...
            <h3>Traitement de votre Test de niveau...</h3>
        ...
        <div id="resultsSection" class="results-section {{ isset($examen) && $examen->score !== null ? 'active' : '' }}">
            @if (isset($examen) && $examen->score !== null)
                <h2>Résultats de votre Test de niveau</h2>
                <div class="warning-alert">
                ...
                @php
                    ...
                    if ($score >= 16) {
                        ...
                        $commentaire =
                            '❌ Résultat insuffisant. Nous vous encourageons à approfondir vos connaissances.';
                    }
                @endphp
...
@push('scripts')
    <script>
        ...
        let examDuration =
        {{ isset($entretien) && $entretien->duree ? $entretien->duree * 60 : 30 * 60 }}; // Durée en secondes
