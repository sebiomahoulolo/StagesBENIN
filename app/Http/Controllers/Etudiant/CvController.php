...
class CvController extends Controller
{
     ...
     */
      public function exportPdf()
      {
           ...
           Log::info("Tentative d'export PDF pour CvProfile ID: " . Auth::user()->etudiant?->cvProfile?->id);
           try {
                ...
                // Génération du PDF
                $pdf = Pdf::loadView('etudiants.cv.templates.default', compact('cvProfile'))
                ...
                Log::info("Export PDF réussi pour {$filename}");

                // Retourne le PDF pour téléchargement
                return $pdf->download($filename);

           } catch (\Exception $e) {
                Log::error("Erreur lors de l'export PDF: " . $e->getMessage(), ['exception' => $e]);
